## myServer: Where reactive values, events, and outputs objects are initiated ##
...
myServer <- function(input, output, session) {
    ...
    ### TAB 1
    # Gene GWAS table 
    source("myServer_Tab1_assocTable.R", local = TRUE)
    output$gene_gwas_data <- asdf
    # Gene NELSON table
    output$gene_nelson_data <- fdsa
    # Disease GWAS table
    ...
    output$gene_disease_nelson_data <- renderDataTable({
        validate(need(rv$diseaseofinterest1,""))
        diseaseofinterest <- rv$diseaseofinterest1
        # Get of list of mapped genes (per diseease "d")
        mapped_genes_nels <- c()
        for(d in diseaseofinterest){
        ...
        mapped_genes <- mapped_genes_nels
        returned_genes_list <- c()
        returned_genes <- for(gene in c(unique(x_expr[,"GENE"]))){
            ifelse(TRUE %in% grepl(gene, mapped_genes), returned_genes_list <- c(returned_genes_list,gene),"")
        }
